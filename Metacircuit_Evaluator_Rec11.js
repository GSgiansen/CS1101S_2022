...
function eval_conditional(comp, env) {
    //display(comp);
    //display(evaluate(conditional_consequent(comp)));
   return is_truthy(evaluate(conditional_predicate(comp), env))
          ? make_literal(evaluate(conditional_consequent(comp), env))
...
function is_conditional(component) {
    display(component);
    return is_tagged_list(component, "conditional_statement") || is_tagged_list(component, "conditional_expression");
}
...
function eval_logical_comp(comp,env){
    if (!evaluate(logical_composition_first_component(comp),env) && logical_symbol(comp) === "&&"){
...
fact(4);
`);*/
//parse_and_evaluate('2 % 5 % 2;');

// no it becomes 2**(5*2), it needs to take into the order 
// in which the power is evaluated
